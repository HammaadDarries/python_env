...
#        planner.save_action_history(FILENAME)
#        planner.generate_plots(FILENAME, observation)
        planner.done_callback(observation)
        observation, reward, done, info = env.reset(poses=np.array([[0, 0, 0]]))
    planner.collision_rate(n_laps)
        

def test_endToEnd_agent():
    map_name = "mco" # "aut", "esp", "gbr", "mco"
    agent_name = FILENAME
    n_test_laps = 50
